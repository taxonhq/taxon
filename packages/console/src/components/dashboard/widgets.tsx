  ...
  return (
    <div className="flex items-center justify-between px-5 py-3 shrink-0"
      ...
      <div className="flex items-center gap-2 min-w-0">
        ...
        <p className="text-[12px] font-semibold truncate" style={{ color: COLORS.ink2 }}>{title}</p>
        {sub && <span className="text-[10px]" style={{ color: COLORS.ink4 }}>· {sub}</span>}
      </div>
      {href && (
        <Link href={href} className="flex items-center gap-0.5 text-[10px] hover:opacity-100 transition-opacity"
...
export function WidgetEmpty({ text }: { text: string }) {
  return (
    <div className="flex-1 flex items-center justify-center text-[12px]" style={{ color: COLORS.ink4 }}>
      {text}
    </div>
  );
}
    ...
    <div className="flex flex-col h-full p-6">
      ...
      <div className="flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] font-semibold uppercase tracking-[0.15em]" style={{ color: COLORS.ink3 }}>
            核心指标 · 标签总数
          </span>
        </div>
        {todayTags && (
      ...
      <div className="mt-6">
        ...
        <div className="flex items-baseline gap-3 mt-2">
          <span className="text-[12px]" style={{ color: COLORS.ink3 }}>标签实例总数</span>
          {todayTags && (
            <span className="text-[11px] tabular-nums" style={{ color: COLORS.ink3 }}>
      ...
      <p className="text-[10px] mt-1 tabular-nums" style={{ color: COLORS.ink4 }}>
        过去 7 天新增标签趋势
      </p>
    </div>
  );
        ...
        <div className="flex-1 grid grid-cols-[1fr,1fr] gap-3 px-5 py-4 min-h-0">
          ...
          <div className="relative flex items-center justify-center min-h-0">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
              <p className="text-[10px] uppercase tracking-wider" style={{ color: COLORS.ink4 }}>总计</p>
              ...
              <p className="text-[10px] mt-1" style={{ color: COLORS.ink3 }}>
          ...
          <div className="flex flex-col gap-1.5 min-h-0 overflow-y-auto pr-1">
            {display.map((t, i) => {
              ...
              return (
                <div key={t.entityType} className="flex items-center gap-2 text-[11px]">
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center justify-center gap-4 px-4 pb-3 shrink-0">
        {[
          ...
          <div key={x.l} className="flex items-center gap-1.5 text-[10px]" style={{ color: COLORS.ink3 }}>
    ...
    <Link href={theme.href} className="flex flex-col h-full p-4 group relative overflow-hidden"
      ...
      <div className="flex items-start justify-between gap-2 shrink-0 relative z-10">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-semibold uppercase tracking-[0.1em]" style={{ color: COLORS.ink3 }}>
            {theme.label}
          </span>
        </div>
        {today && today.comparePct !== 0 && (
          <span className="text-[10px] font-bold tabular-nums flex items-center gap-0.5" style={{
      ...
      <div className="flex-1 flex flex-col justify-center relative z-10">
        ...
        {today && (
          <p className="text-[10px] mt-1.5 tabular-nums" style={{ color: COLORS.ink3 }}>
            今日 +{today.today}
          </p>
        )}
      </div>
        ...
        <ul className="flex-1 overflow-y-auto px-2 py-2 space-y-0.5">
          {activity.map((ev, i) => (
            <li key={`${ev.kind}-${ev.time}-${i}`}
              className="px-3 py-2 rounded-md text-[11px] transition-colors hover:bg-white/[0.025] animate-slide-up"
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="font-mono text-[10px] truncate" style={{ color: COLORS.ink }}>{ev.tagName}</span>
                <span className="text-[10px] ml-auto shrink-0 tabular-nums" style={{ color: COLORS.ink4 }}>
                  {timeAgo(ev.time)}
                </span>
              </div>
              <p className="text-[10px] mt-0.5 truncate font-mono" style={{ color: COLORS.ink3 }}>
...
export function HealthBar({ health }: { health: HealthInfo | null }) {
  const cells: Array<{ label: string; value: React.ReactNode }> = [
    ...
    {
      label: "服务版本",
      value: <span className="font-mono text-[13px] font-semibold" style={{ color: COLORS.ink }}>
        {health?.version ? `v${health.version}` : "—"}
      </span>,
    },
    {
      label: "Node.js",
      value: <span className="font-mono text-[12px]" style={{ color: COLORS.ink2 }}>
        {health?.nodeVersion ?? "—"}
      </span>,
    },
  ];
      ...
      <div className="flex-1 grid grid-cols-4 min-h-0">
        {cells.map((c, i) => (
          <div key={c.label} className="flex flex-col justify-center gap-1 px-4"
            ...
            <p className="text-[10px] font-semibold uppercase tracking-[0.1em]" style={{ color: COLORS.ink4 }}>
              {c.label}
            </p>
            <div>{c.value}</div>
          </div>
